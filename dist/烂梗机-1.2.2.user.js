...
// ==/UserScript==
// 版本历史：
// 1.2.2 hybrid（DOM+协议双引擎互斥切换）：修复双源叠加/房间号/安全阀持久化/断线重连/开关联动；详见 CHANGELOG.md
...
(function () {
  ...
  // ---------------- 斗鱼 STT（danmuproxy 自连 / 页面 Hook 共用） ----------------
  ...
  function _pbStr(f) {
    if (!f || !f.bytes) return '';
    try { return new TextDecoder().decode(f.bytes); } catch (_) { return ''; }
  }
  ...
  function _tarsValue(u8, pos, le, depth) {
    ...
    switch (h.type) {
      ...
      case TARS.STRING1: {
        ...
        node.str = new TextDecoder().decode(u8.slice(h.next + 1, h.next + 1 + n));
      ...
      case TARS.STRING4: {
        ...
        node.str = new TextDecoder().decode(u8.slice(h.next + 4, h.next + 4 + n));
        node.next = h.next + 4 + n;
        return node;
      }
      case TARS.SIMPLELIST: {
        // 内层 head（应为 INT8 tag0）+ u32 size + bytes
        const ih = _tarsHead(u8, h.next);
        if (!ih || ih.type !== TARS.INT8) return null;
        if (ih.next + 4 > u8.length) return null;
        const n = dv.getUint32(ih.next, le);
        if (n > u8.length || ih.next + 4 + n > u8.length) return null;
        node.bytes = u8.slice(ih.next + 4, ih.next + 4 + n);
        node.next = ih.next + 4 + n;
        return node;
      }
      case TARS.LIST: {
        if (h.next + 4 > u8.length) return null;
  ...
  // ---------------- 页面 WS Hook 基座（虎牙/抖音/斗鱼-Hook 路线共用，只读不发） ----------------
  ...
  // 虎牙 Tars 字节序：协议文档口径为 little-endian（真机矩阵文档为唯一权威）→ LE 优先；
  // BE 兜底以防文档口径偏差，解出首条弹幕后锁定（锁定结果写事件日志，真机首验可确认）。
  // cmd 为小整数单字节，与字节序无关；iUri=1400 是判别锚点。
  var __lgjHuyaLE = null;
  function __lgjHuyaDecode(u8) {
    var tries = __lgjHuyaLE === null ? [true, false] : [__lgjHuyaLE];
    for (var i = 0; i < tries.length; i++) {
      try {
        ...
        return { anyFrame: true, danmus: [] }; // 结构自洽但本帧无弹幕（心跳/回执/其它 uri）
      } catch (_e) { /* 忽略 */ }
    }
    return { anyFrame: false, danmus: [] };
  }
  ...
  function engageProto() {
    ...
    // 纯自连路线（无 Hook 兜底）必须先有房间号
    if (!room && routes.indexOf('hook') < 0) { __lgjEngaging = false; setStatus('协议：无法识别房间号', 'err'); return Promise.resolve(false); }
    stopProtocol();
    setStatus('协议：连接中…', '');
