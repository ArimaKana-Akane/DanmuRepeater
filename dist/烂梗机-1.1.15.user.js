...
(function () {
    ...
    // ============================================================
    // 平台感知帧选择（1.1.15 重写，替代 1.1.14 的无差别 iframe 排除）：
    // - B 站弹幕区在同源 iframe（live.bilibili.com/blanc/...）内渲染（实测），
    //   必须允许脚本在该 iframe 内运行；B 站主文档（房间外壳页）退出，防双实例
    // - 其他平台的 iframe 一律退出（保留 1.1.14 防多实例语义）
    // - 跨域 iframe：location 比较本身安全，仅读 top 引用不触异常
    ...
    if (IS_TOP_FRAME) {
        // B 站主文档无弹幕 DOM（在 blanc iframe 内），直接退出避免空转双实例
        if (location.hostname.includes('bilibili.com')) return;
    } else if (!IS_BLANC_FRAME) {
