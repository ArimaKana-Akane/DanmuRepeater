...
- **抖音进场提示被当弹幕采集并发送**（第三轮真机复测发现，同属 P0）：
  抖音把进场提示渲染成独立条目「`<昵称> 来了`」/「`<昵称>等<N>人来`」，并把平台合规宣导
  也放在同一容器里。这些条目的文本**干净、长度正常、无任何系统关键词**，且它们**没有**
  真弹幕的文本节点 `.webcast-chatroom___content-with-emoji-text` → legacy 的 `danmuText`
  选择器抽不到文本、回退整条 `textContent` → 真机实测已被选为「下次发送」
  （`选择 | "失败的m 来了"`、`next = "FourMinuteMile 来了"`）。修复（两层）：
  - 结构层（抖音）：条目内**有** `content-with-emoji-text` → 直接放行（不做文本判断）；
    没有该节点时**不**一律判系统，仅在文本也明确像进场/系统时才判——若页面改名该节点，
    「一律判系统」会把**全部弹幕**误杀，比漏过滤更严重（刻意做成 fail-safe）。
  - 文本层：新增「整串 = 昵称段 + 可选的『等N人』 + 来了」模式。
    **误杀边界是本条最大的风险**：「我来了」「他来了」是玩家真弹幕，第一版判据把
    `我来了`/`他来了` 误杀了；最终判据要求无「等N人」时昵称段**≥2 字符且不以单一人称/
    指示代词开头**（我/你/他/她/它/咱/俺/这/那/又/才/刚/快/就可/真/原来），
    并用负向先行断言排除含冒号的串（`昵称：内容` 格式的真弹幕）。
    已补 14 例「不得误杀」断言锁死该边界。
  - 顺带补齐既有缺口：「进直播间了」变体（原模式只认「进入了直播间」）。
- **B 站 `getDanmuInfo` 风控失败原因不可读**：真机测得该接口**间歇性**返回 `code=-352`
  （同页 `nav`/`room_init` 均 `code=0`；页面内**不带签名**的裸请求同样 `-352`，
  但脚本自身也曾走到 `auth-timeout`，即两条失败路径都会出现）。
  脚本原先只把它混进 `danmuinfo--352` 抛错，语义不明。修复：区分并标注为
  「风控拒绝(-352)」，便于判断是「换 IP/登录态可解」还是脚本侧协议问题。
  **注意**：这不改变失败本身——该环境下 B 站协议直连确实拿不到弹幕数据，回落 DOM 是正确行为。
...
- **单测辅助函数修正**：旧 `tBytes()`/`tList()` 用「裸 u32 小端」构造帧，与旧解码器
  **共享同一错误假设**——这正是 1.2.2 的 76 个单测全绿而真机虎牙零弹幕的原因。
  ...
  第二/三轮又补：虎牙礼物/贵族进场文本兜底、抖音进场（含 14 例「不得误杀」边界）等。全套件 76 → **94 例**。
- 产物守卫：`虎牙 LE 优先口径` 标记改为 `虎牙 BE 优先口径` + 新增 `虎牙类型化 size（BE）`
  标记，防止字节序回归；第二/三轮再补（虎牙包裹层结构判据、真弹幕放行优先、
  礼物文本兜底、贵族进场文本兜底、抖音真弹幕文本节点判据、抖音进场文本兜底、
  进场变体、发送者字典上限、共享 UTF-8 解码器、B 站风控可读化）。守卫 66 → **87 项**。
...
- 新增 3 项守卫标记（`虎牙主动注册 live:0`、`虎牙注册帧发送`、`虎牙类型化 size`）。
  最终守卫总数 66 → **87 项**（含第二/三轮虎牙包裹层、抖音文本节点、B 站风控可读化等）。

### 真机模式矩阵（第三轮，四平台 × 三模式）

在真实登录态的隔离 profile 里，对每个平台依次切 `auto` / `protocol` / `dom` 并读取实际数据源：

| 平台 | auto | protocol | dom | 协议路线实际结果 |
|---|---|---|---|---|
| 斗鱼 9999 | ✅ 协议接管（Hook） | ✅ 协议接管（Hook） | ✅ 回落 DOM | **成功**——`metaMap` 的 `boost=1.31` 证明协议数据真实进入候选加权 |
| 虎牙 520258 | 连接中→回落 | ✅ 回落（15s 退避） | ✅ DOM | 失败（uri1400 从未出现，见「已知边界」） |
| B 站 261095 | ✅ 回落（60s 退避） | ✅ 回落（15s 退避） | ✅ DOM | 失败（`getDanmuInfo` 间歇 `-352` 风控 / `auth-timeout`） |
| 抖音 | ✅ DOM | — | ✅ DOM | 未测（房间离线，判据改用真机 HTML 离线断言验证） |

三项状态机行为在此矩阵中得到确认：
1. **`dom` 模式状态行立即同步**（本轮修复项）——四平台均正确显示「DOM 采集」；
2. **会话级停用生效**——B 站连续 3 次无产出后 `sp=true`，状态显示「协议路线本会话已停用」，
   不再周期性重连抖动；
3. **用户显式切模式可解除停用**——`switchMode()` 后 `sp=false / fb=0`，尊重用户意图。
